...
Filled by the applicant without logging. Recreate from Typeform confirmation or skip.

---

## Q16 — What is your expected project timeline or duration?

~6.5 weeks solo FTE over a ~7–8 week wall-clock window, keyed to four funded milestones:

- **M3 — Live Reclaim SDK integration** (2 weeks) — browser SDK + minimal backend proxy, real providers (GitHub, Twitter/X), end-to-end verified-human proofs from real HTTPS sources.
- **M4 — On-chain anchor submission flow** (1.5 weeks) — browser-side `anchor(proof)` via Auro wallet, closing the read/write gap on our deployed zkApp.
- **M5 — Demo, docs polish, community outreach** (1 week) — 60–90s demo video, README/docs polish, blog post + community-call walkthrough per program expectations.
- **M6 — Upstream PR to Reclaim Mina repo** (2 weeks) — contribute our in-circuit ECDSA-secp256k1 verifier + test vectors back to reclaimprotocol/mina-sdk-onchain-integration, plus third-party security review pass.

Work starts within one week of grant confirmation. Progress shared publicly per program expectations — weekly update on X/GitHub, blog post at midpoint, live community-call demo at close.

---

## Q17 — List your intended milestones and target KPIs.

**M3 — Live Reclaim SDK integration** (2 wks)
- Deliverable: browser SDK + minimal backend proxy; real GitHub + Twitter/X providers wired end-to-end.
- KPIs: ≥2 HTTPS providers live; ≥10 real attested proofs generated end-to-end (non-fixture); browser prove time ≤90s for attested mode; public test URL with "Verify with GitHub" flow.

**M4 — On-chain anchor submission via Auro** (1.5 wks)
- Deliverable: creator UI calls `anchor(proof)` on our deployed zkApp via Auro wallet.
- KPIs: `proofCount` on-chain ≥5 anchors generated by users other than the developer; verifier UI correctly identifies them as anchored-latest; tx inclusion rate 100% across the sample.

**M5 — Demo video, docs, community outreach** (1 wk)
- Deliverable: 60–90s demo video, README polish, blog post, community-call walkthrough.
- KPIs: video published (YouTube unlisted → public at submission); blog post on personal site + cross-posted; ≥1 Mina community call walkthrough; repo stars/forks ≥25 / ≥3 (soft KPI for traction signal).

**M6 — Upstream PR to Reclaim Mina repo** (2 wks)
- Deliverable: PR to `reclaimprotocol/mina-sdk-onchain-integration` contributing the in-circuit ECDSA verifier + documented test vectors; external security review pass.
- KPIs: PR opened + CI green; ≥1 Reclaim maintainer review comment cycle completed; security review report published in our repo.

**Cross-cutting KPIs** — test suite stays at 100% pass; browser verification stays <1s end-to-end; zkApp remains operational on devnet through the grant window.

---

## Q18 — Is there anything else you'd like us to know about your project?

Everything is already live before we submit: repo at github.com/Kvkthecreator/zk-proof-of-origin, zkApp deployed to Mina devnet at `B62qpPxWR3QXCuA4bEZTmtz5ZmFBfFPpZyCrxo4NuWS6uJ9nbHgNoVU`, live browser demo at kvkthecreator.github.io/zk-proof-of-origin, 18/18 tests passing, full decision log and analysis docs in the repo. The grant funds the next 6.5 weeks, not a blank-page kickoff. Happy to jump on a call any time.
